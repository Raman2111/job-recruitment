...
function SingleJobPage() {
  ...
  const deleteJob = () => {
    axios
      .delete('/api/jobs/' + params?.job_id, { headers: { Authorization: getToken() } })
      .then((res) => {
  ...
  return (
    <SingleJobPageLayouts page='single-job-page'>
          ...
          <Col sm={12} md={8} className='p-3'>
              ...
              <Col xs={4} className='my-3'>
                ...
                <div xs={9}>
                  <span className='small'>
                    {moment(job?.endDate).format('MMM Do YYYY') ?? 'unavailable'}
                  </span>
                </div>
              </Col>
            ...
            {getDecoded()?.role === 'company' && (
              <>
                ...
                <Button size='' variant='outline-danger' onClick={() => deleteJob()}>
                  <span>Delete</span>
                </Button>
      ...
      {getDecoded()?.role === 'company' &&
        getDecoded()?.id.toString() === job?.company?._id.toString() && (
          <>
